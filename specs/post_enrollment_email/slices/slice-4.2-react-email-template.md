...
backend/
└── email/
    ├── handler.ts         # Lambda handler (from Slice 4.1)
    ├── render.ts          # Email rendering + event router (NEW)
    ├── types.ts           # Event and email data types (UPDATE)
    ├── emails/
    │   ├── enrollment-email.tsx  # Email template (NEW)
    │   └── index.ts
    ├── components/
    │   ├── header.tsx
    │   ├── footer.tsx
    ...
    ├── package.json       # NEW
    ├── tsconfig.json      # NEW
    └── .react-email/      # Auto-generated by dev server
```
...
- Handler receives SNS event → calls `renderEmailFromEvent()` → sends via SES
...
6. **Test button:** Developer clicks "Start Learning" in preview. Opens course URL in new tab. ✓
